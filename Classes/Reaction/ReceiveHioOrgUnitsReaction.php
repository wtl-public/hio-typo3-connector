...
class ReceiveHioOrgUnitsReaction implements ReactionInterface
{
    ...
    {
        return 'tx-hio_typo3_connector-orgunits';
    }
        ...
        if (is_array($payload['data'] ?? false)) {
            ...
            foreach ($payload['data'] as $value) {
                $this->eventDispatcher->dispatch(
                    new ReceiveHioOrgUnitEvent(
                        OrgUnitDto::fromArray($value),
                        $storagePid
                    )
                );
            }
        }
        return $this->createJsonResponse(['status' => 'Organisation units imported']);
    }
