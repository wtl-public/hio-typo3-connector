...
class ReceiveHioPatentsReaction implements ReactionInterface
{
    ...
    {
        return 'receive-hio-patents';
    }

    public static function getDescription(): string
    {
        return 'LLL:EXT:hio_typo3_connector/Resources/Private/Language/locallang.xlf:receive-hio-patents_reaction';
    }

    public static function getIconIdentifier(): string
    {
        return 'tx-hio_typo3_connector-patents';
    }
        ...
        if (is_array($payload['data'] ?? false)) {
            ...
            foreach ($payload['data'] as $value) {
                $this->eventDispatcher->dispatch(
                    new ReceiveHioPatentEvent(
                        PatentDto::fromArray($value),
                        $storagePid
                    )
                );
            }
        }
        return $this->createJsonResponse(['status' => 'Patents imported']);
    }
